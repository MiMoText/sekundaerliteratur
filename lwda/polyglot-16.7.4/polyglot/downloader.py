...
class Downloader(object):
  ...
  def list(self, download_dir=None, show_packages=False,
    ...
    for category in categories:
      ...
      for info in sorted(getattr(self, category)(), key=str):
        ...
        name = textwrap.fill('-' * 27 + (info.name or info.j),
                             75, subsequent_indent=27*' ')[27:]
        print('  [%s] %s %s' % (prefix, info.j.ljust(20, '.'), name))
  ...
  def _download_package(self, info, download_dir, force):
    ...
    self._status_cache.pop(info.j, None)
    ...
    if info.filename.endswith('.zip'):
      ...
      if info.unzip or os.path.exists(os.path.join(zipdir, info.j)):
      ...
      for msg in self.incr_download(info_or_id, download_dir, force):
        ...
        if isinstance(msg, ErrorMessage):
          ...
          if not quiet:
            ...
            if choice in ['y', 'Y']:
              if not self.download(msg.package.j, download_dir,
                                   quiet, force, prefix,
        ...
        if not quiet:
          ...
          if isinstance(msg, StartCollectionMessage):
            show('Downloading collection %r' % msg.collection.j)
            prefix += '   | '
            print(prefix)
  ...
  def status(self, info_or_id, download_dir=None):
    ...
    # Handle collections:
    if isinstance(info, Collection):
      pkg_status = [self.status(pkg.j) for pkg in info.packages]
    ...
    # Handle packages:
      ...
      if download_dir != self._download_dir:
        ...
        if info.j not in self._status_cache:
          self._status_cache[info.j] = self._pkg_status(info,
                                                        filepath)
        return self._status_cache[info.j]
  ...
  def supported_tasks(self, lang=None):
    ...
    if lang:
      ...
      return [x.j.split('.')[0] for x in collection.packages]
    else:
      return [x.name.split()[0] for x in self.collections() if Downloader.TASK_PREFIX in x.j]
    ...
    while True:
      ...
      for info in sorted(getattr(self._ds, 'packages')(), key=str):
        if self._ds.status(info) == self._ds.STALE:
          stale_packages.append((info.j, info.name))
